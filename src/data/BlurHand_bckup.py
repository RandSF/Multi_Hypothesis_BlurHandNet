...
class BlurHand(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.data_split == 'train' or self.data_split == 'test':
            ...
            # if mano_param_past is not None:
            #     pose_past = mano_param_past['pose']
            #     shape_past = mano_param_past['shape']
            #     transl_past = mano_param_past['trans']
            # else:
            #     pose_past = np.zeros((mano.orig_joint_num*3), dtype=np.float32) 
            #     shape_past = np.zeros((mano.shape_param_dim), dtype=np.float32)
            #     transl_past = np.zeros((3), dtype=np.float32)
            
            # if mano_param is not None:
            #     pose = mano_param['pose']
            #     shape = mano_param['shape']
            #     transl = mano_param['trans']
            # else:
            #     pose = np.zeros((mano.orig_joint_num*3), dtype=np.float32) 
            #     shape = np.zeros((mano.shape_param_dim), dtype=np.float32)
            #     transl = np.zeros((3), dtype=np.float32)

            # if mano_param_future is not None:
            #     pose_future = mano_param_future['pose']
            #     shape_future = mano_param_future['shape']
            #     transl_future = mano_param_future['trans']
            # else:
            #     pose_future = np.zeros((mano.orig_joint_num*3), dtype=np.float32) 
            #     shape_future = np.zeros((mano.shape_param_dim), dtype=np.float32)
            #     transl_future = np.zeros((3), dtype=np.float32)

            # mano_pose_orig = np.stack([pose_past, pose, pose_future], 0).astype(np.float32)
            # mano_shape_orig = np.stack([shape_past, shape, shape_future], 0).astype(np.float32)
            # mano_transl_orig = np.stack([transl_past, transl, transl_future], 0).astype(np.float32)

            # for render
            # Fx, Fy = data['cam_param']['focal']
            # Px, Py = data['cam_param']['princpt']
            ...
            joint_hm = img2hm(joint_img, self.opt_params['output_hm_shape'])
            ...
            targets = {'joint_img': joint_img, 'joint_cam': joint_cam, 'mano_joint_cam': mano_joint_cam, 
                       ...
                       'joint_hm': joint_hm, 'mesh_cam': mesh_cam}
