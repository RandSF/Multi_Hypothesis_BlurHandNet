...
class Runner():
    ...
    def _prepare_training(self):
        ...
        ema = ExponentialMovingAverage(model.parameters()) if self.opt['use_ema'] else None
        ...
        if self.local_rank == 0 and self.opt.get('continue_train', False):
            start_epoch, model, optimizer, ema, scheduler, scaler = self.continue_train(model, optimizer, ema, scheduler, scaler)
    ...
    def continue_train(self, model: DistributedDataParallel, optimizer: torch.optim.Optimizer, ema,
                        scheduler: torch.optim.lr_scheduler.LRScheduler, scaler: GradScaler):
        ...
        if self.opt['use_ema']: ema.load_state_dict(ckpt['ema'])

    
        # if self.local_rank == 0:self.logger.info('Load checkpoint from {}'.format(ckpt_path))
        
        return start_epoch, model, optimizer, ema, scheduler, scaler
    ...
    def step(self, inputs, targets, meta_info):
        ...
        if self.opt['use_ema']: self.ema.update(self.model.parameters())
