...
class Unfolder(nn.Module):
    ...
    def forward(self, feat_blur, feat_pyramid):
        ...
        joint_img_e1 = soft_argmax_2d(pred_heatmap_e1)
        joint_img_md = soft_argmax_2d(pred_heatmap_md)
        joint_img_e2 = soft_argmax_2d(pred_heatmap_e2)
        ...
        return torch.stack([feat_joint_e1, feat_joint_md, feat_joint_e2], dim=1), torch.stack([joint_img_e1, joint_img_md, joint_img_e2],dim=1) / 32
...
class Decoder(nn.Module):
    ...
    def forward(self, feat_blur, feat_pyramid):        
        ...
        heatmap = self.conv_proj(x).reshape(-1, self.num_joints, *self.output_hm_shape[1:])
