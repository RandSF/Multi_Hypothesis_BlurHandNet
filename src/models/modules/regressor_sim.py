...
class Regressor(nn.Module):
    ...
    def forward(self, feat_blur, feat_joint, joint_img):
        ...
        # mano pose parameter regression
        K, B, T, E = feat_joint.shape
        # joint_img = joint_img.flatten(-2).unsqueeze(-2).unsqueeze(0).expand(K, -1, -1, J, -1)
        joint_img = joint_img[None].expand(K, -1, -1, -1, -1)
        feat_one = torch.cat((feat_joint, joint_img.flatten(-2)), -1)   # [K, B, T, -1]
