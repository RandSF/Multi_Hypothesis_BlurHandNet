...
class Integral(nn.Module):
    ...
    def generate_integral_preds_2d(self, heatmaps):
        ...
        # softmax
        if self.config['MODEL']['integral']['use_softmax']:
            # heatmaps[heatmaps<0.1] = -1e9
            heatmaps = (heatmaps.reshape((batch_size, num_joints, -1))*self.config['MODEL']['integral']['alpha']).double()
            heatmaps = F.softmax(heatmaps, 2)
            heatmaps = heatmaps.reshape((batch_size, num_joints, height, width)).float()
        elif self.config['MODEL']['integral']['use_pow']:
            temp = self.config['MODEL']['integral']['pow_temp']
