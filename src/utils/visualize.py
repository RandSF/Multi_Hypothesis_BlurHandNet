...
def save_obj_with_time(v, f, t, file_name='output.obj'):
    color_seq=[' 0 100 200', '100 100 200', '150 100 200']
    obj_file = open(file_name, 'w')
    for i in range(len(v)):
        obj_file.write('v ' + str(v[i][0]) + ' ' + str(v[i][1]) + ' ' + str(v[i][2]) +  color_seq[t] + '\n')
    for i in range(len(f)):
        obj_file.write('f ' + str(f[i][0]+1) + '/' + str(f[i][0]+1) + ' ' + str(f[i][1]+1) + '/' + str(f[i][1]+1) + ' ' + str(f[i][2]+1) + '/' + str(f[i][2]+1) + '\n')
    obj_file.close()

# def plot_seq(img_path, output_file, dpi=None):
#     fig = plt.figure()
...
#     plt.savefig(output_file, transparent=True, dpi=dpi)
#     plt.close()

def plot_seq(obj_pathes, output_file, dpi=None, gt = False):
    plt.ioff()
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    if gt:
        color_seq=np.array([[100, 200, 50], [100, 200, 100], [100, 200, 150]])/255
    else:
        color_seq=np.array([[0, 100, 200], [100, 100, 200], [150, 100, 200]])/255
    
    for t, objp in enumerate(obj_pathes):
        obj = ObjFile(objp)
        ax.plot_trisurf(obj.nodes[:,0],obj.nodes[:,1],obj.nodes[:,2], triangles=obj.QuadToTria(), color=color_seq[t], alpha=0.4)
