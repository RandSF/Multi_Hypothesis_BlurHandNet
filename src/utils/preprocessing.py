...
def process_db_coord(joint_img, joint_cam, joint_valid,
    ...
    joint_img[:,0] = joint_img[:,0] / input_img_shape[1]
    joint_img[:,1] = joint_img[:,1] / input_img_shape[0]
    joint_img[:,2] = (joint_img[:,2] / (bbox_3d_size / 2) + 1)/2.
    
    # check truncation
    joint_trunc = joint_valid * ((joint_img[:,0] >= 0) * (joint_img[:,0] < 1) * \
                (joint_img[:,1] >= 0) * (joint_img[:,1] < 1) * \
                (joint_img[:,2] >= 0) * (joint_img[:,2] < 1)).reshape(-1,1).astype(np.float32)
...
def process_db_coord_pcf(joint_img_p, joint_img_c, joint_img_f,
    ...
    joint_img_p[:,0] = joint_img_p[:,0] / input_img_shape[1]
    joint_img_p[:,1] = joint_img_p[:,1] / input_img_shape[0]
    joint_img_p[:,2] = (joint_img_p[:,2] / (bbox_3d_size / 2) + 1)/2.
    ...
    joint_img_c[:,0] = joint_img_c[:,0] / input_img_shape[1]
    joint_img_c[:,1] = joint_img_c[:,1] / input_img_shape[0]
    joint_img_c[:,2] = (joint_img_c[:,2] / (bbox_3d_size / 2) + 1)/2.
    ...
    joint_img_f[:,0] = joint_img_f[:,0] / input_img_shape[1]
    joint_img_f[:,1] = joint_img_f[:,1] / input_img_shape[0]
    joint_img_f[:,2] = (joint_img_f[:,2] / (bbox_3d_size / 2) + 1)/2.
    
    # check truncation
    joint_trunc_p = joint_valid_p * ((joint_img_p[:,0] >= 0) * (joint_img_p[:,0] < 1) * \
                (joint_img_p[:,1] >= 0) * (joint_img_p[:,1] < 1) * \
                (joint_img_p[:,2] >= 0) * (joint_img_p[:,2] < 1)).reshape(-1,1).astype(np.float32)
    joint_trunc_c = joint_valid_c * ((joint_img_c[:,0] >= 0) * (joint_img_c[:,0] < 1) * \
                (joint_img_c[:,1] >= 0) * (joint_img_c[:,1] < 1) * \
                (joint_img_c[:,2] >= 0) * (joint_img_c[:,2] < 1)).reshape(-1,1).astype(np.float32)
    joint_trunc_f = joint_valid_f * ((joint_img_f[:,0] >= 0) * (joint_img_f[:,0] < 1) * \
                (joint_img_f[:,1] >= 0) * (joint_img_f[:,1] < 1) * \
                (joint_img_f[:,2] >= 0) * (joint_img_f[:,2] < 1)).reshape(-1,1).astype(np.float32)
...
def process_human_model_output(human_model_param, cam_param, do_flip,
    ...
    # check truncation
    joint_trunc = ((joint_img[:,0] >= 0) * (joint_img[:,0] < 1) * \
                (joint_img[:,1] >= 0) * (joint_img[:,1] < 1) * \
                (joint_img[:,2] >= 0) * (joint_img[:,2] < 1)).reshape(-1,1).astype(np.float32)
